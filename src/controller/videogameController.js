...
async function getAllVideogames (model1,model2){
    // try {
        ...
        let response = await axios.get(`${API}/games?key=${API_KEY}&page_size=25&page=${numPage}`)
...
async function getVideogamesByName(req,model1,model2){
        ...
        let games = [];
        let response = await axios.get(`${API}/games?search=${name}&search_precise=true&key=${API_KEY}&page_size=15`)
            response.data.results.forEach(element => {
                games.push( {
                    name:element.name,
                    id:element.id,
...
async function getGameDetails(req,model1, model2){
    // try {
        ...
        if(idGame.length >=36) { //lamo a la DB si tiene mas de 35 char
            ...
            let response = await axios.get(`${API}/games/${idGame}?key=${API_KEY}`),
